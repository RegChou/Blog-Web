package com.muxui.blog.service.monitor.util;
...
public class RuntimeUtil {
     ...
     * 获取系统属性
     */
    public static Result getProperty() {
...
//            obj.put("javaLibraryPath",props.getProperty("java.library.path"));// 加载库时搜索的路径列表
     ...
     * 获取内存信息
     */
    public static Result getMemory() {
        JSONObject obj = new JSONObject();
        try {
...
//            obj.put("free", free); // 当前内存剩余量
     ...
     * 获取网卡信息
     */
    public static Result getEthernet() {
        JSONObject jsonObject = new JSONObject();
        try {
            Sigar sigar = new Sigar();
